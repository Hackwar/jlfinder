...
abstract class JHtmlFilter
{
	...
	public static function slider($options = array())
	{
		...
		$query->clear()
			...
			->from($db->quoteName('#__jlfinder_taxonomy') . ' AS t')
			->join('INNER', $db->quoteName('#__jlfinder_taxonomy') . ' AS c ON c.parent_id = t.id')
			->where('t.parent_id = 1')
			->where('t.state = 1')
		...
		foreach ($branches as $bk => $bv)
		{
			...
			$query->select('e.title AS parent_title')
				->join('LEFT', $db->quoteName('#__jlfinder_taxonomy', 'e') . ' ON ' . $db->quoteName('e.id') . ' = ' . $db->quoteName('t.parent_id'));
