...
define('FINDER_PATH_INDEXER', JPATH_PLUGINS . '/system/jlfinder/admin/helpers/indexer');
...
/**
	...
	protected function getListQuery()
	{
		...
		if (!empty($this->searchquery->filters))
		{
			...
			$node_ids = array();
			array_walk_recursive($groups, function($a) use (&$node_ids) { $node_ids[] = $a; });

			$query->join('INNER', $db->quoteName('#__finder_taxonomy_map') . ' AS t ON t.link_id = l.link_id')
				->where('t.node_id IN (' . implode(',', array_unique($node_ids)) . ')');
		...
		if (!empty($this->searchquery->date2))
		{
			...
			{
				$query->where($db->quoteName('l.end_date') . ' <= ' . $date2);
			}
		 ...
		 */
		if (empty($this->includedTerms) && (!$this->searchquery->empty || ($this->searchquery->empty && $this->searchquery->input)))
		{
		...
		{
			foreach ($this->requiredTerms as $key => $terms)
			{
				if (count($terms))
				{
