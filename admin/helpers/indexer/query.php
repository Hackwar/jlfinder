...
JLoader::register('FinderHelperLanguage', JPATH_SITE . '/administrator/components/com_finder/helpers/language.php');
...
class FinderIndexerQuery
{
	...
	public function __construct($options)
	{
		...
		$this->mode = !empty($options['mode']) ? $options['mode'] : 'AND';
		...
		if (isset($options['when2']) && !empty($options['date2']))
		{
	...
	protected function processDynamicTaxonomy($filters)
	{
		...
		$query->select('t1.id, t1.title, t2.title AS branch')
			->from($db->quoteName('#__jlfinder_taxonomy') . ' AS t1')
			->join('INNER', $db->quoteName('#__jlfinder_taxonomy') . ' AS t2 ON t2.lft < t1.lft AND t1.rgt < t2.rgt')
			->where('t1.state = 1')
			->where('t1.access IN (' . $groups . ')')
			->where('t1.id IN (' . implode(',', $filters) . ')')
			->where('t2.level = 1')
