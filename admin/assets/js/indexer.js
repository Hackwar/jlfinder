	...
	var handleResponse = function (json, resp) {
		try {
			...
			if (json.start) {
				...
				if (jQuery('#finder-debug-data').length) {
					for (var context in json.pluginState) {
						jQuery('#finder-debug-data').append('<dt>' + context + '</dt><dd id="finder-' + context.replace(/\s+/g, '-').toLowerCase() + '">' + json.pluginState[context].offset + ' of ' + json.pluginState[context].total + '</dd>');
					}
				}
			}
			...
			if (jQuery('#finder-debug-data').length) {
				for (var context in json.pluginState) {
					jQuery('#finder-' + context.replace(/\s+/g, '-').toLowerCase()).text(json.pluginState[context].offset + ' of ' + json.pluginState[context].total);
				}
			}
