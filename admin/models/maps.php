...
class FinderModelMaps extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query = $db->getQuery(true)
			...
			->from($db->quoteName('#__jlfinder_taxonomy', 'a'))
			->where('a.parent_id != 0');

		// Join to get the map links.
		$stateQuery = $db->getQuery(true)
			->select('m.node_id')
			->select('SUM(l.published=1) AS count_published')
			->select('SUM(l.published=0) AS count_unpublished')
			->from($db->quoteName('#__finder_taxonomy_map', 'm'))
			->innerJoin($db->quoteName('#__finder_links', 'l') . ' ON l.link_id = m.link_id')
			->group('m.node_id');
		...
		{
			$query->leftJoin('#__jlfinder_taxonomy AS b ON b.lft <= a.lft AND a.rgt <= b.rgt')
	...
	{
		return new MapTable($this->getDbo());
	}
	...
	public function purge()
	{
		$db = $this->getDbo();
		$query = $db->getQuery(true)
			->delete($db->quoteName('#__finder_jltaxonomy'))
